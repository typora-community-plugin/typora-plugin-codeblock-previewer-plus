...
- **全屏模式** — 可选的全屏查看方式：滚轮缩放、拖拽平移，单击遮罩（或按 Esc）关闭
...
| 设置             | 说明                                         | 默认值     |
| ---------------- | -------------------------------------------- | ---------- |
| `LANGS`          | 启用预览按钮的代码块语言（逗号分隔）          | `flow,mermaid,sequence` |
| `MODE`           | 预览按钮打开的查看方式：浮动窗口 / 全屏遮罩    | 浮动窗口   |
| `FLOATING_WIDTH` | 浮动窗口的初始宽度（占视口宽度的百分比）       | 45         |
| `FLOATING_HEIGHT`| 浮动窗口的初始高度（占视口高度的百分比）       | 60         |
